...
export default function MarketIntelligence() {
  ...
  const p = productPages.marketIntelligence
    ...
    <ProductPageTemplate breadcrumbs={p.breadcrumbs} sections={p.sections} header={p.header} relatedProducts={p.related}>
      ...
      <Section id="insight">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {p.insightList.map((item, i) => (
            <Card key={i}>
              <div className="text-2xl mb-2">{item.icon}</div>
              <h4 className="font-bold text-shade-600 text-sm">{item.title[lang]}</h4>
              <p className="text-xs text-gray-500 mt-1">{item.desc[lang]}</p>
            </Card>
          ))}
        </div>
      </Section>
